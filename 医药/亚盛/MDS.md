---
CreateTime: 2026-07-17 20:38:45
LastUpdate: 2026-07-17 20:38:45
---
...
| MDS干细胞类型                     | 主要生存依赖                   | 占比 |
| --------------------------------- | ------------------------------ | ---- |
| CMP型、较原始的长期HSC型          | BCL-2                          | 52%  |
| └ TP53 突变（10-15% 的 MDS 携带） | 对BCL-2天然耐药                |      |
| GMP型、粒-单核分化型              | 更依赖TNF–NF-κB、MCL-1、BCL2A1 | 48%  |
|                                   |                                |      |
| 获得性耐药后转换                  | 部分CMP→GMP/BCL-XL 或 MCL-1    |      |

> 占比：更早期的 ASH 2018 摘要（101例）报道的比例为 38% vs 62%，但最终发表在 *Nature Medicine* 的 123 例队列为 52% vs 48%。
...
BCL-2抑制剂杀的是所有高度依赖BCL-2维持生存的细胞；MDS异常原始细胞通常依赖性更强，所以优先被杀，但部分正常造血干/祖细胞也会成为附带损伤

异常MDS细胞：接近凋亡临界点，高度依赖BCL-2，正常造血细胞：通常离凋亡临界点更远，依赖BCL-2程度较低

主要伤害的是正常造血祖细胞，而非所有成熟血细胞

| 正常细胞                | BCL-2抑制剂影响                               |
| ----------------------- | --------------------------------------------- |
| 正常造血干细胞          | 相对耐受，但并非完全不受影响                  |
| 粒细胞祖细胞            | 可能被抑制，导致中性粒细胞恢复缓慢            |
| 正常淋巴细胞，尤其B细胞 | 可直接受影响                                  |
| 成熟中性粒细胞          | 多数不是直接大量杀死，而是骨髓补充不足        |
| 成熟红细胞              | 没有线粒体，基本不是BCL-2药物直接靶点         |
| 血小板                  | 主要依赖BCL-XL，选择性BCL-2抑制剂直接影响较小 |

MDS患者也容易出现骨髓抑制

1. MDS患者原本正常造血储备就少
2. 骨髓内大量细胞本身属于异常克隆。
3. 清除异常细胞后，正常干细胞未必能立刻补上。
4. 阿扎胞苷同时也会影响正在增殖的正常祖细胞。
5. 疾病环境中的IL-3等信号还可能使非恶性造血干/祖细胞上调BCL-2，从而变得更容易被Venetoclax杀伤
...
3. Lisaftoclax在早期的药理分析中，看到了BCL-2释放BIM后，上调Noxa，抑制了MCL-1通路，后期临床表现为对部分Venetoclax耐药患者仍有疗效；那么Sonrotoclax和Venetoclax在药理分析时，有上调Noxa或其他方式的通路，进而抑制MCL-1吗？
4. mds中，bcl-2+aza，如果有一个因为毒性药减量，那么减哪个好，我的意思是，哪个对mds更重要？
5. 为什么HI比mCR更容易OS获益，我清除了更多的异常造血干细胞，那么正常的分化细胞不是会更多起来吗，对身体不是更好吗？
6. BCL-2杀了骨髓异常细胞，需要多久能恢复生产正常细胞？
7. AZA+Lisaftoclax对比AZA单药，在血液数据上，会有什么不同？
8. AZA和Venetoclax抑制的是同一种细胞吗？
9. 如果ven+aza早期研究，是不是更接近这两个药联用的天花板，那么BCL-2和aza的天花板差不多就是这个了，跟aza单药也没什么区别，bcl-2还有意义吗？
10. Lisaftoclax在MDS上的所有临床数据
11. MDS的CR能否传导到OS
12. 改善mCR、真的对改善OS有益处吗？
13. 为什么缓解率意味着可以移植；为什么不让MDS患者转化为AML后再治疗，我记得AML的药也有很多？
14. 为什么tp53特别是双等位突变、复杂核型的MDS患者在临床中难以缓解？
